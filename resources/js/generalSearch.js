document.addEventListener("DOMContentLoaded", () => {
    ...
    async function searchPost(searchText) {
            ...
            if (Array.isArray(elements) && elements.length > 0) {
                ...
                if (elements.length > 3) {
                    ...
                    <div class="flex items-center justify-between mt-5">
                        <p class="text-[#012F4A] dark:text-neutral-200 font-bold">${title}:</p>
                    ...
                    resultContent += `<p class="text-[#012F4A] dark:text-neutral-200 font-bold mt-2">${title}:</p>`;
                }
                limitedElements.forEach(element => {
                    ...
                    resultContent += `<a href="${url}" class="block border-b border-[#E6E5DE] p-2 pl-5 dark:text-white">${elementName}</a>`;
                    hasContent = true;
                });
            }
        }
        ...
        if (!hasContent) {
            generalResultContainer.innerHTML = "<p class='text-center pt-3 dark:text-white'>No hi ha resultats</p>";
        } else{
