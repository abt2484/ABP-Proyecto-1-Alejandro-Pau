<tr data-clickable-element="true" class="mb-10 bg-white border-b border-[#AFAFAF] text-[#0F172A] hover:bg-[#eeeeee65] hover:transition-all dark:bg-neutral-800 dark:border-neutral-600 dark:text-white dark:hover:bg-neutral-700 cursor-pointer">
    ...
    <td class="text-center px-3">
        <p class="truncate">{{ $externalContact->phone ?? "-" }}</p>
        @if ($externalContact->email)
            <a href="mailto:{{ $externalContact->email }}" target="_blank" class="truncate text-[#df6400]">{{ $externalContact->email }}</a>
        @else
            <p class="truncate">-</p>
        @endif
    </td>
    ...
    <td class="px-3">
        <div class="flex items-center justify-center gap-2 p-2">
            ...
            <form action="{{ $externalContact->is_active ? route('external-contacts.deactivate', $externalContact) : route('external-contacts.activate', $externalContact) }}" method="post" class="w-full sm:w-full md:w-auto lg:w-auto">
                @csrf
                @method('PATCH')
                <button type="submit" class="confirmable w-24 flex justify-center gap-3 {{ $externalContact->is_active ? "text-white bg-red-600 rounded-lg p-2 font-semibold cursor-pointer hover:bg-red-800 hover:transition-all" : "text-white bg-green-600 rounded-lg p-2 font-semibold cursor-pointer hover:bg-green-700 hover:transition-all" }}" data-confirm-message="{{ $externalContact->is_active ? 'Estàs segur que vols desactivar aquest contacte?' : 'Estàs segur que vols activar aquest contacte?' }}">
