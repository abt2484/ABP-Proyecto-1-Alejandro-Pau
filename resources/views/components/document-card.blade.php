<div data-clickable-element="true" class="shadow-md border border-[#AFAFAF] bg-white rounded-[15px] p-5 min-w-[220px] mb-5 flex flex-col gap-5 dark:bg-neutral-800 dark:border-neutral-600 dark:text-white cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-700">
            ...
            <div class="flex flex-row justify-between gap-2">
                <div class="flex flex-row items-center gap-5">
                    ...
                    <div class="flex flex-col items-start">
                        <p class="text-[#AFAFAF] text-xs md:text-sm font-semibold">{{ $document->type ? strtoupper($document->type) : ""}}</p>
    ...
    <div class="flex flex-row gap-5 justify-end">
        <a href="{{ route('documents.edit', $document) }}"
            class="w-full sm:w-full md:w-[33%] flex gap-3 bg-white text-[#011020] rounded-lg p-2 font-semibold items-center justify-center cursor-pointer border-1 border-[#AFAFAF] dark:bg-neutral-800 dark:border-neutral-500 dark:text-white dark:hover:bg-neutral-600">
            <svg class="w-6 h-6">
        ...
        <a href="{{ route('documents.download', ['path' => $document->path]) }}"
            class="w-full sm:w-full md:w-[33%] flex gap-3 bg-[#FF7E13] text-white rounded-lg p-2 font-semibold items-center justify-center cursor-pointer hover:bg-blue-800 transition-all">
            <svg class="w-6 h-6">
                <use xlink:href="#icon-download"></use>
            </svg>
            Descarregar
        </a>
        <form action="{{ route('documents.destroy', $document) }}" method="post" class="w-full sm:w-full md:w-[33%]">
            @csrf
            @method("DELETE")
            <button type="submit"
                class="confirmable w-full sm:w-full md:w-full flex justify-center gap-3 text-white bg-red-600 rounded-lg p-2 font-semibold cursor-pointer hover:bg-red-800 hover:transition-all"
