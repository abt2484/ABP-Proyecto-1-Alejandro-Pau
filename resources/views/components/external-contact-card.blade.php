    ...
    <div>
            ...
            <div class="flex flex-row justify-between gap-2">
                <div class="flex flex-row items-center gap-5">
                    ...
                    <div class="flex flex-col items-start">
                        <p class="text-[#AFAFAF] text-sm3 font-semibold">{{ $externalContact->category ? strtoupper($externalContact->category) : ""}}</p>
                        <a href="{{ route("external-contacts.show", $externalContact) }}" class="text-[#012F4A] font-bold text-[20px]">{{ $externalContact->contact_person }}</a>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-3 ml-3 space-y-3 ">
            <div class="flex items-center text-[#011020]">
                ...
                <p class="ml-2">{{ $externalContact->company_or_department }}</p>
            </div>
            ...
            <div class="flex items-center text-[#011020]">
                ...
                <p class="ml-2">{{ $externalContact->contact_person ?? 'No disponible' }}</p>
            </div>
            <div class="flex items-center text-[#011020]">
                <svg class="w-7 h-7">
                    <use xlink:href="#icon-mail"></use>
                </svg>
                <p class="ml-2">{{ $externalContact->email }}</p>
            </div>
        </div>
    </div>
