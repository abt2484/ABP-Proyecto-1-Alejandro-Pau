...
<div class="min-w-fit w-9/10 mx-auto flex flex-col mb-7 gap-10 dark:text-white">
    ...
    <form action="{{ route('evaluations.store', $user->id) }}" method="POST">
        @csrf
        @method("POST")
        <div class="border border-[#AFAFAF] bg-white rounded-[15px] p-10 flex flex-col gap-5 dark:bg-neutral-800 dark:border-neutral-600">
            <div class="flex flex-col w-full">
                ...
                @for ($i = 1; $i <= 20; $i++)
                <div class="flex flex-row items-center pl-5 h-20 border-b-1 border-b-[#AFAFAF] dark:border-b-neutral-600">
                    <div class="w-1/2 font-bold text-lg">{{ $questions[$i-1] }}</div>
                    <div class="flex-1 flex justify-between">
                        @for ($j = 0; $j < 4; $j++)
                        <div class="flex-1 border-1 border-[#AFAFAF] dark:border-neutral-600 h-20 flex items-center justify-center" data-question="p{{ $i }}" data-value="{{ $j }}">
